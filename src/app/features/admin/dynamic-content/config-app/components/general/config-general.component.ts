        ...
        <div class="flex justify-between items-center flex-wrap gap-4">
          <div>
            ...
            <p class="text-slate-500 dark:text-slate-400 mt-1 text-sm">
              Configuración básica que se utiliza en diferentes partes del sitio web.
            </p>
          </div>
          <p-button
            label="Guardar Sección General"
            icon="pi pi-check"
            [loading]="saving()"
            (onClick)="saveConfig()"
            styleClass="p-button-emerald"
          ></p-button>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-2">
          <div class="flex flex-col gap-2">
            <label
              ...
              >Nombre de la Aplicación / Marca</label
            >
            <input
              id="config-nombre"
              pInputText
          ...
          <div class="flex flex-col gap-2">
            <label
              for="config-telefono"
              class="text-sm font-semibold text-slate-700 dark:text-slate-300"
              >Teléfono General</label
            >
            <input
              id="config-telefono"
              pInputText
              type="text"
              [(ngModel)]="settings().telefono"
          ...
          <div class="flex flex-col gap-2 md:col-span-2">
            ...
            <div class="flex items-center gap-4">
              ...
              @if (settings().logo_url) {
                <div
                  class="w-14 h-14 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden bg-slate-50 dark:bg-slate-950 flex items-center justify-center p-1 flex-shrink-0"
                >
                  <img
