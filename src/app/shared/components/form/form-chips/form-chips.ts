...
@Component({
    ...
    template: `
        <div class="mb-4">
            ...
            <p-autocomplete class="w-full text-black border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-200"
                [ngModel]="control().value()" 
                (ngModelChange)="control().controlValue.set($event)" 
                (keydown.enter)="$event.preventDefault()" 
                [inputId]="uid" 
                multiple fluid [typeahead]="false" [placeholder]="placeholder()"
